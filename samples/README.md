...
> **SDK 2.1.4:** Все примеры подключают SDK как `<PackageReference Include="GrossGeo.SDK.Stub" Version="2.1.4" />`
...
Если User Panel не запущен, SDK работает из локального кэша (DPAPI) до 7 дней (для `Machine`-режима — до 30).
По истечении grace-периода `IsValid = false` и плагин переходит в Free-режим. Это позволяет проверять
graceful degradation без запущенного User Panel.
