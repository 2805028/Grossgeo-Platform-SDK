...
проверке лицензии, и это единственное, что решает срок дискового кэша SDK. У всех привязок отдельно
есть серверная аренда (lease, 24 ч + 3 дня grace, у `Concurrent` короче): она переживает отсутствие
сети иначе — на пути **User Panel**, которая при недоступном сервере проверяет её локально
