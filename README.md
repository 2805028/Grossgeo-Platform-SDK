...
> вывод «значит поддерживается» был неверен: по данным Autodesk сборка под `net8` не совместима
> с AutoCAD 2027 и требует пересборки под `net10.0-windows`. Дело не в хосте .NET 10 как таковом
> — на AutoCAD 2026.1.2 и 2025 U1.4, тоже на хосте .NET 10, `net8`-сборки грузятся штатно.
> Обещание из более раннего текста снято этим же выпуском SDK.

Сам пакет SDK собран под `net48` и `net8.0-windows`; из проекта под `net10.0-windows` он
подключается штатно — отдельной сборки под .NET 10 не требуется.
...
> U1.4, тоже на хосте .NET 10, net8-сборки грузятся штатно). Ранняя редакция этого текста
> читала «загружается» как «поддерживается» и была неверна. Для `net10` — `R26.0`, это
> AutoCAD 2027. Какая серия окажется у следующей версии AutoCAD, мы не выводим формулой —
> пару «серия → год» назначает Autodesk (см. `AutoCadSeries`), поэтому и вы не угадывайте: когда
> она станет известна, значение обновится в этой документации.
>
> Пример релиза продукта, собранного под AutoCAD 2027 (`net10.0-windows`):
>
> ```json
> {
>   "version": "2.0.0",
>   "distributionType": "Bundle",
>   "netloadDllPath": "Contents/MyPlugin.dll",
>   "minAutoCADVersion": "R26.0",
>   "maxAutoCADVersion": "R26.0",
>   "targetPlatforms": ["AutoCAD", "Civil3D"],
>   "supportedOS": ["Win64"]
> }
> ```
